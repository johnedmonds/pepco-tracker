...
public class PepcoOutageController {
	...
	public static class OutageRevisionWrapper {
		...
		public OutageRevisionWrapper(final OutageRevision revision) {
			...
			this.earliestReport = revision.getOutage().getEarliestReport()
					.getTime();
			...
			this.status = WordUtils.capitalize(revision.getStatus().toString()
	...
	Collection<OutageRevisionWrapper> getOutagesAsOf(
		...
		final Collection<AbstractOutageRevision> revisions = this.outageDao
				.getOutagesAsOf(
						new Timestamp(timestamp < 0 ? new Date().getTime()
								: timestamp), OutageRevision.class);
